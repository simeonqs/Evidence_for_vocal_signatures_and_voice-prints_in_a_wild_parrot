...
sim.sn.data.time = function(settings = list(N_ind = 3,
  ...
  for(ind in 1:settings$N_ind){
    ...
    for(obs in 1:N_obs){
      ...
      dat = rbind(dat, 
                  rnorm(settings$N_var, means_ind, settings$sigma_obs))
  ...
  if(plot_it) plot(dat[,1], dat[,2], pch = 16, col = alpha(inds, time_saver/20))
  
  # Make into distance matrix
  names(dat) = paste0('x', 1:ncol(dat))
  m = as.matrix(dist(dat))
  d = m.to.df(m, inds, time_saver = time_saver)
