...
# Description: Running new date model on real data. 
# source('ANALYSIS/CODE/social networks model/13_run_date_model_real.R')
# >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
...
path_out = 'ANALYSIS/RESULTS/social networks model/real_model_date_new.RData'
