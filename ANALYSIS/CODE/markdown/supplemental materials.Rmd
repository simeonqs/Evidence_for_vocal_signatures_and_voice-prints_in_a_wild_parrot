...
For the results presented in the paper there was no overlap with zero for any of the pDFAs. For full distributions and sample sizes see Figure \@ref(fig:figure-pdfa).
...
When sub-setting the data to only include females from the square the results for growly to tonal fell towards chance-level, but all other results remained stable with overlap with zero less than 2%. For full distributions and sample sizes see Figure \@ref(fig:figure-pdfa-sub-f).

```{r figure-pdfa-sub-f, include=TRUE, fig.align="center", fig.cap='pDFA results for females from the square. Densities of the differences between the trained and random DFAs. N is number of individals for which there was enough data available.', fig.height=5, fig.width=7, echo=FALSE}
knitr::include_graphics(path_pdf_pdfa_sub_f)
```

\pagebreak

When sub-setting the data to only include males from the square the results for both cross-call type models fell towards chance-level. For full distributions and sample sizes see Figure \@ref(fig:figure-pdfa-sub-m).

```{r figure-pdfa-sub-m, include=TRUE, fig.align="center", fig.cap='pDFA results for males from the square. Densities of the differences between the trained and random DFAs. N is number of individals for which there was enough data available.', fig.height=5, fig.width=7, echo=FALSE}
knitr::include_graphics(path_pdf_pdfa_sub_m)
```

\pagebreak

When sub-setting the data to only include females from the square the results for the cross call type models remained stable but overlap with zero increased to 6-7%. For full distributions and sample sizes see Figure \@ref(fig:figure-pdfa-perm).

```{r figure-pdfa-perm, include=TRUE, fig.align="center", fig.cap='pDFA results with randomisation withing square and park only. Densities of the differences between the trained and random DFAs. N is number of individals for which there was enough data available.', fig.height=5, fig.width=7, echo=FALSE}
